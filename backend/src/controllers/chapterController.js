...
const getChapters = async (req, res, next) => {
  try {
    ...
    const [filteredChapters, totalChapters] = await Promise.all([
      Chapter.find(filter)
        .skip(skip)
        .limit(limit)
        .sort({ createdAt: -1 })
...
// //       // // Generate cache key
// //     const cacheKey = cacheService.generateCacheKey('/api/v1/chapters', req.query);

// //     const cachedData = await cacheService.get(cacheKey);
// //     if (cachedData) {
// //       return res.status(200).json(cachedData); // ✅ Properly returns parsed JSON, not a Promise
// //     }

// //     const query = {};
// //     const chapter = await Chapter.find(query);

// //     // Cache the fresh response
// //     await cacheService.set(cacheKey, chapter);
...
// Incorrect (this will send a Promise instead of parsed JSON)
// if (cachedData) {
//   return res.status(200).json(cachedData); // ✅ This is now correct
// }
